# random forest for training chrombpnet-classification model
...
final_path = '../../datasets/tissue_specific/chrombpnet_results/'
output_path = '../../datasets/tissue_specific/chrombpnet_prediction_results/'
...
for tissue in compare_tissue_list:
    for model_size in model_size_list:
        ...
        feature_list = []
        labels = np.array(train_table['META_DATA'].astype("int"))
        for i in range(train_table.shape[0]):
            sample_feature = []
            sample_feature.append(train_table['log_counts_diff'][i])
            sample_feature.append(train_table['log_probs_diff_abs_sum'][i])
            sample_feature.append(train_table['probs_jsd_diff'][i])
        ...
        feature_list = []
        labels = np.array(test_table['META_DATA'].astype("int"))
        for i in range(test_table.shape[0]):
            sample_feature = []
            sample_feature.append(test_table['log_counts_diff'][i])
            sample_feature.append(test_table['log_probs_diff_abs_sum'][i])
            sample_feature.append(test_table['probs_jsd_diff'][i])
            feature_list.append(sample_feature)
        features = np.array(feature_list)
