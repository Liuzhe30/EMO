...
class Dense3dLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope(name):
          ...
          name="kernel",
          shape=[hidden_size, self.num_attention_heads * hidden_size],
          initializer=self.initializer)

            if self.use_bias:
                self.b = tf.compat.v1.get_variable(
            name="bias",
            shape=[self.num_attention_heads * hidden_size],
    ...
    def call(self, input_tensor):
        ...
        self.w, [hidden_size, self.num_attention_heads, self.num_attention_heads * self.size_per_head])
        if self.head_first:
        ...
        if self.use_bias:
            ...
            self.b, [1, self.num_attention_heads, 1, self.num_attention_heads * self.size_per_head])
            else:
                reshape_b = tf.reshape(
            self.b, [self.num_attention_heads, self.num_attention_heads * self.size_per_head])
            ret += reshape_b
...
class Dense3dProjLayer(tf.keras.layers.Layer):
    ...
    def call(self, input_tensor):
        ...
        hidden_size = self.num_attention_heads * self.size_per_head       
        reshape_w = tf.reshape(
        self.w, [self.num_attention_heads, self.num_attention_heads * self.size_per_head, hidden_size])
