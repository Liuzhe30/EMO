...
def generate_middle_output(model_size, layer_name, tissue): 
    ...
    model.load_weights('model/weights_e2e/' + model_size + '/' + model_size + '_' + tissue + '_trained_weights.tf').expect_partial()
    ...
    np.save('middle_output_tissue_e2e/' + model_size + '_' + tissue + '_middleoutput.npy', npy_out)
    np.save('middle_output_tissue_e2e/' + model_size + '_' + tissue + '_label.npy', label)
