...
for tissue in compare_tissue_list:
    for model_size in ['large','middle','small']:
        for splittype in ['train','test']:
            data_all = pd.read_pickle('../../datasets/tissue_specific/' + model_size + '/' + splittype + '_' + model_size + '_' + tissue + '.pkl')[['phenotype_id','variant_id','tss_distance','slope','bulk']]
            new_df = pd.DataFrame(columns=['variant_id', 'slope', 'result_before', 'result_after'])
            ...
            print(new_df.head())
            new_df.to_pickle('enformer_results_slope/' + splittype + '_' + model_size + '_' + tissue + '.pkl')