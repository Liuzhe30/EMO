...
gtex_bulk_list = ['Colon_Transverse', 'Thyroid', 'Nerve_Tibial']
    
file_path_1 = 'E:/SNP_eQTL_rawdata/GTEx_Analysis_v8_EUR_eQTL_all_associations/'
file_path_2 = 'E:/SNP_eQTL_rawdata/GTEx_v8_finemapping_CAVIAR/caviar_output_GTEx_LD/'
for bulk in gtex_bulk_list:
    for chr_num in range(1, 23): # ignore chrX
        ...
        print(new_df_up)
        new_file_up = 'D:/eQTL_SNP/dataset/0_ppc_0.9/' + bulk + '_' + str(chr_num) + '.pkl'
        new_df_up.to_pickle(new_file_up)
        '''
        new_df_down = new_df_down.reset_index() 
        print(new_df_down)
        new_file_down = 'D:/eQTL_SNP/dataset/0_ppc_0.001/' + bulk + '_' + str(chr_num) + '.pkl'
