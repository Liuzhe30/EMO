...
def bigbird_block_sparse_attention(query_layer,
    ...
    blocked_query_matrix = tf.reshape(query_layer, (-1, h, m // wm, wm, h*d))
    blocked_key_matrix = tf.reshape(key_layer, (-1, h, n // wn, wn, h*d))
    blocked_value_matrix = tf.reshape(value_layer, (-1, h, n // wn, wn, h*d))
    gathered_key = tf.reshape(
      tf.gather(blocked_key_matrix, rand_attn, batch_dims=2, name="gather_key"),
      (-1, h, m // wm - 2, r * wn, h*d))  # [b, h, n//wn-2, r, wn, -1]
    ...
    first_product = tf.multiply(first_product, 1.0 / np.sqrt(h*d))
    ...
    second_product = tf.multiply(second_product, 1.0 / np.sqrt(h*d))
    second_product += (1.0 -
    ...
    #     ==> [b, h, m//wm-4, wm, 3*wn]
    inner_band_product = tf.multiply(inner_band_product, 1.0 / np.sqrt(h*d))
    rand_band_product = tf.einsum(
      "BHLQD,BHLKD->BHLQK", middle_query_matrix, gathered_key[:, :, 1:-1]
      )  # [b, h, m//wm-4, wm, -1] x [b, h, m//wm-4, r*wn, -1]
    #     ==> [b, h, m//wm-4, wm, r*wn]
    rand_band_product = tf.multiply(rand_band_product, 1.0 / np.sqrt(h*d))
    first_band_product = tf.einsum(
      "BHLQD,BHKD->BHLQK", middle_query_matrix, blocked_key_matrix[:, :, 0]
      )  # [b, h, m//wm-4, wm, -1] x [b, h, wn, -1] ==> [b, h, m//wm-4, wm, wn]
    first_band_product = tf.multiply(first_band_product, 1.0 / np.sqrt(h*d))
    last_band_product = tf.einsum(
      "BHLQD,BHKD->BHLQK", middle_query_matrix, blocked_key_matrix[:, :, -1]
      )  # [b, h, m//wm-4, wm, -1] x [b, h, wn, -1] ==> [b, h, m//wm-4, wm, wn]
    last_band_product = tf.multiply(last_band_product, 1.0 / np.sqrt(h*d))
    ...
    second_last_rand_pad = tf.concat(
      [tf.ones_like(second_last_product[:, :, :, :4 * wn]),
       rand_mask[:, :, -1]], 3)
    second_last_product = tf.multiply(second_last_product, 1.0 / np.sqrt(h*d))
    second_last_product += (
    ...
    last_product = tf.einsum(
      ...
      key_layer)  # [b, h, wm, -1] x [b, h, n, -1] ==> [b, h, wm, n]
    last_product = tf.multiply(last_product, 1.0 / np.sqrt(h*d))
    ...
    context_layer = tf.reshape(context_layer, (-1, h, m, h*d)) * from_mask
